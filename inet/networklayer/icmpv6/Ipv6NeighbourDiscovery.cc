...
void Ipv6NeighbourDiscovery::processIpv6Datagram(Packet *packet)
{
     ...
     */
    bool pAddRandomDelays = par("addRandomDelays").boolValue();
    ...
    case Ipv6NeighbourCache::REACHABLE:
        ...
        if (pAddRandomDelays)
            sendDelayed(packet, uniform(0.3, 0.5), "ipv6Out"); // was 0.3-1
        else
            send(packet, "ipv6Out");
        break;
    case Ipv6NeighbourCache::DELAY:
        ...
        if (pAddRandomDelays)
            sendDelayed(packet, uniform(0.3, 0.5), "ipv6Out");
        else
            send(packet, "ipv6Out");
        break;
...
void Ipv6NeighbourDiscovery::initiateNeighbourUnreachabilityDetection(Neighbour *nce)
{
    ...
    if (par("addRandomDelays").boolValue()) {
        if (nce->nudTimeoutEvent) {
            cancelEvent(nce->nudTimeoutEvent);
            nce->nudTimeoutEvent = nullptr;
        }
    }
